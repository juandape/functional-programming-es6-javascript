...
function ferraris(obj) {
  const enzo = obj.filter((item) => item.brand === "Ferrari");
  return enzo;
}
...
function nineties(obj) {
  const age = obj.filter((item) => item.model > 1980 && item.model < 1990);
  return age;
}
...
function list(obj) {
  let newList = [];
  obj.forEach((item) => {
    newList.push(`This ${item.brand} from ${item.model} costs $${item.price}`);
  });
  return newList;
}
